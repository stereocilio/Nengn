...
TIEMPO_PRUEBA = 20 ##TIEMPO DE LA PRUEBA EN SEGUNDOS
...
def loop():
    ...
    if (state_loop == 2):
        logo()
        estado = raw_input("Nueva Muestra? y/n: ")
        if (estado == 'y'):
            limpiar()
            reset()
            state_loop = 1
...
def profilePatient():
    global name_folder
    logo()
    number_input= raw_input("Id: ")
    name_input = raw_input("Nombre: ")
    lastname_input = raw_input("Apellido: ")
    age_input = raw_input("Edad: ")
    height_input = raw_input("Altura cm: ")
    weight_input = raw_input("Peso kg: ")
    name_folder = number_input+"_"+name_input+"_"+lastname_input


    createPatient = raw_input("¿Los datos son Correctos? y/n: ")

    if createPatient.lower() == "y":
        limpiar()
        createFolder()
        createLog(number_input, name_input, lastname_input, age_input, height_input, weight_input)
...
def selectTest():
    ...
    state_test= raw_input("Presione <enter> para comenzar:")
    if (cond1 == True):
        vx0 = []
        yaw0 = []
        pitch0 = []
...
def collect():
    ...
    log_test = open(name_folder+'/'+"C1"+"_"+str(date.day)+'-'+str(date.month)+'-'+str(date.year)+'_'+str(date.hour)+':'+str(date.minute)+str(date.second)+'.log', 'a')
#    arduino.write(b'9')
    i = 0
    t = 0
...
#####Textos de Aviso
banner = colored(centerify('Full Axis V0.4',80), 'white', attrs=['reverse'])
